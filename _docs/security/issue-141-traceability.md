...
| Redacted logs/metrics/traces/audits/browser artifacts | Specs 06, 07, 10 | `core.redaction`, `core.audit`, `course_management.observability`, redaction canary tests, `uv run --frozen python scripts/ci.py security-artifact-scan` |
| Dependency/container/security checks | Specs 01, 07, 10 | frozen `uv.lock`, non-root `Dockerfile`, `pip-audit` lock-digest artifact, cryptography advisory remediation, `uv run --frozen python scripts/ci.py security-check`, `verification-container` |
