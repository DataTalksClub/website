...
- Research baseline, refreshed 2026-09-13: origin/main `2a356fdc5116c978b8b6ddbf9c46c3c5b534591e`, "Merge docs-refresh-345: Refresh the community-base migration-plan snapshot after the #345 django lane (#380)", merged 2026-09-12. This plan does not authorize rewriting or discarding merged commits. Re-record the merged baseline before every implementation; branch drift invalidates inventories.
- Published releases as of 2026-09-13 span two lines. The maintenance line `v0.3.6`, `v0.3.7` and `v0.3.9` (verified tag commits `a4d26323` and `00b624e1` for v0.3.6 and v0.3.9) includes kernel/config/API/jobs/mail/Studio/content sync, the exported test doubles, Studio nested destination groups with `STUDIO_EXTRA_CSS` (v0.3.6) and the C6.2/C6.2a Relay clients (v0.3.7, v0.3.9); `git ls-tree` at those tags shows no domain directories under `community_base/`, so they remain the adoption-safe pins. The main-line release `v0.3.8` (tag commit `6fe8c585`) additionally contains the domain subpackages (accounts, community, events, curriculum, coursework and siblings), and their migrations carry the provisional naming (`0001_provisional_initial.py` and siblings) while C3.7 and C4.3 are open. Tagging a release containing provisional migrations is forbidden by the shared plan rules; the observation is recorded for the plan owner, and no site may install those subapps until donor compatibility passes and C5.3 v0.6.0 is published. Deployed dependencies pin published tags only; never package main, a branch, or an editable source.
- C3.7 requires prepared AISL and DTC identity donor evidence; C4.3 requires the prepared AISL events donor. C5.3 v0.6.0 is the future adoption-ready domain release, after those donor checkpoints. Curriculum (C5.1a-d) and coursework (C5.2a-e) are merged on package main; their kept-label migrations remain provisional and stay untagged until donor compatibility passes. No task may assume an API that is not in a published tag; main-only capability needs a release first.
...
Dated snapshot of the migration rollout. Community-base [`docs/plan/STATUS.md`](https://github.com/DataTalksClub/community-base/blob/main/docs/plan/STATUS.md) is authoritative for live status across all four repositories, including AISL progress (A2.1, A2.3 and A6.2 in progress in that repository; the A1.2 remainder is tracked as AISL #1629).

- Done: D0.1a (merged `f8f68c46`), D0.1b (#355), D0.2 (#354), D1.1 (#350), D1.2a (#368), D1.2b (#370).
- Merged but dev-deploy blocked: D0.1c (merged `3fb9db07`) and D1.2ca (#372, branch `issue-372`, gates green) - blocked on a fully green main CI run: the django lane of website#345 merged as `fc534fcf` and the complete django suite runs green corpus-less, while scheduled full regressions remain red on the Playwright lane (latest checked run 2026-09-12 22:20Z; design-parity fixes with the refreshed `playwright_tests/test_deployed_smoke.py` copy pins stay in flight on the main checkout). The worker self-check IAM ask is resolved (aws-infra#49 closed, grant merged as aws-infra `3acefe43`, effective with the next website-terraform apply).
- In progress: D2.1 (#377; engineer handoff frozen for the independent tester gate in the worktree), D2.2a (#379; fix round three over the sync parsers and migration tree), D3.1 (#334; no movement since 2026-09-08).
- Waiting: D0.1d (#385), D1.2cb (inside the #372 umbrella), D1.3, D2.2b (#382), D2.2c (#384), the D3.2 freeze and later phases.
- Releases and pins: v0.3.6 through v0.3.9 published 2026-09-12/13 (see the release-boundary bullet above; the maintenance line is the adoption-safe line). DTC still pins v0.3.0 from D0.1a; pin bumps ride their owning issues (P15).
...
| Events | `events/models.py` contains Event, EventContent, EventSpeaker, EventLink, five Q&A models, historical source/revision/slot/displacement/total state, EventRegistrantIdentity, EventRegistration, EventRegistrantInterestSignal, EventRegistrantImportProgress. (`EventAlias` was retired: an Event is addressed only by its id, and no alias mechanism is retained.) Events are already DB-owned. D4 is a lossless replacement of an active domain, not simply a one-time GitHub import. Include current registrants, provenance and aggregate arbitration. |
| Event identity | `events/models.py:resolve_public_id`, `resolve_uuid`, `create_event_identity`; spec05 requires immutable UUID management identity and numeric public identity | Package integer primary keys require an explicit site UUID-to-package-ID extension. Management APIs stay UUID-addressed; the legacy date/title and UUID public redirects are retired (an Event is addressed only by its id), so no compatibility-alias behavior remains to preserve. |
...
Depends on published C5.3 for implementation; inspect earlier without installing untagged code. Read `events/models.py`, `events/services.py`, `events/qna/services.py`, `scripts/prod/identity_manifest.py`, `scripts/prod/registrant_import.py`, current registration/import modules located by `rg --files events`, `events/tests/`, [`05-events-registration-email.md`](../specs/05-events-registration-email.md) and [`event-qna-integration.md`](../architecture/event-qna-integration.md).
