...
- Research baseline, refreshed 2026-09-12: origin/main `950065e0da6bf44853b9c23b4d6785d436c6e4cc`, "Restore the green release matrix (#345)", merged 2026-09-12. This plan does not authorize rewriting or discarding merged commits. Re-record the merged baseline before every implementation; branch drift invalidates inventories.
- Published package v0.3.5 (tag `v0.3.5`, merged as `739aa28`, package PR #207, 2026-09-12) includes kernel/config/API/jobs/mail/Studio/content sync and the exported test doubles; point releases v0.3.1 through v0.3.5 add no domain subpackages (verified at the tag: no domain directories under `community_base/`). Package main also contains newer domain capability work with provisional kept-label migrations. Deployed dependencies pin published tags only; never package main, a branch, or an editable source.
