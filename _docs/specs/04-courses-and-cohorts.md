...
`RegistrationCampaign` carries the public registration total as plain current-state fields, not a
versioned overlay: `registration_baseline_count` is, at most, a fixed historical figure recorded
once (for a campaign whose registration history predates this database's `CourseRegistration`
rows, e.g. a legacy CMP export), `registration_native_start_at` is the instant native rows became
the complete record, and `registration_baseline_cohort` is which cohort that baseline was recorded
for -- it stops applying the moment the campaign promotes a different cohort, so a baseline never
carries onto whatever edition registers next.

`courses.services.registration_counts.public_course_registration_count(campaign)` returns the
baseline (when it still applies) plus a live count of native `CourseRegistration` rows created on
or after the native boundary, for the campaign's current cohort. It returns `None` only when the
campaign has no current cohort; every other campaign renders its live total. The copied
registration template stays byte-identical; the only copied-view seam is this one query.
