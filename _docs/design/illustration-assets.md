...
[`website-illustrations` skill](../../.agents/skills/website-illustrations/SKILL.md)
for generation, chroma removal, strict border repairs and browser acceptance.
It is the maintained production playbook: built-in imagegen with actual image
references, complete illustrations with individually distinct blurbs, a uniform
saturated chroma backdrop, then transparency added by removing only that backdrop
and its contaminated fringe. The user's chroma preference replaces the earlier
native-transparency-first and pure-white-key procedures.

For a course robot, use `home-step-2.webp` as the drawing-style reference and the
historical robot-reading course image as the subject reference; exclude the
historical banner's lettering. Match each existing target to its own composition
anchor. A light-only repair does not authorize new dark companions.

For the homepage's light Step 1 and Step 3 border repair, Step 2 guides only edge
appearance. Keep each target's own blurb shape, interior texture and foreground;
only its outer border may change. The skill specifies pixel protection and
comparison, genuine alpha checks and desktop/mobile review on the light page.

The [2026-09-09 edge-repair record](references/blurb-edge-repair.json) is historical
evidence, not the current procedure. Its cloud-only outputs, white-key dark
sources and aggressive alpha cutoff must not be copied as the default recipe.
The [2026-09-10 light-border repair](references/light-border-repair.json) records
the complete illustration edits, measured chroma removal, exact pixel protection
and accepted desktop/mobile evidence for light Steps 1 and 3. Its measured key
values apply to those sources; check new outputs independently.
...
First inspect the generated PNG. Continue after the skill's chroma removal and
edge checks pass, with the cloud preserved and white drawing fills opaque. For an
accepted course image already at 1024 × 1024, encode without resizing:
...
resizing and transparent padding. This is for new assets, not border repairs
whose original drawing must retain its exact pixels and coordinates. Do not use
`!`, which can stretch the drawing:
...
their foregrounds and interior texture; only the outer border may change.
...
```text
Render for the light page: generate the complete drawing with its own irregular,
softly painted pale lavender cloud against the uniform chroma backdrop specified
below. Preserve the approved calm Step 1 expression, the hero's previously
approved composition with its visible second hand, and Step 3's small filled
black hand exactly as it appears in the approved light anchor.
```

For the dark pass, append:

```text
Render natively for the dark page: use a subdued indigo watercolor cloud whose
edge will blend into the dark page like the approved dark hero while keeping the
foreground colours close to the light anchor. Use the
approved dark green and indigo accents from the correction ledger. Generate the
complete drawing with its own cloud against the uniform chroma backdrop specified
below; its removal will let the actual dark page surface show through. No
light-mode background, checkerboard, rectangular plate, edge
...
For either theme, append the skill's uniform saturated chroma requirements. Use
`#ff00ff` unless it occurs in the artwork; do not substitute white or a dark
opaque background. Remove the exterior key and inspect the resulting transparency
according to the skill before encoding. Light and dark assets must each look
clean on their respective real page/card/band backgrounds, without pixel noise,
halo, fringe, outline or plate.

Generate at the slot's aspect ratio. If the raw output needs a final canvas
normalisation, first remove the chroma backdrop, then preserve the recovered
alpha channel and centre it without stretching. Do not resample the preserved
original drawing during a border repair:
...
Replace the names and dimensions for the other slots. The finished image must
have no rectangular edge, glow or coloured fringe; correct the key removal or regenerate the source
...
Keep screenshots below `.tmp/`, then run the focused homepage tests:
