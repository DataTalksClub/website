...
- CMP `hwN` slugs on the 2026 module cohorts are rewritten to `homework-0N` so `prepare_local_course_modules` (`preserve_existing_records=True`) adopts them by slug and does not replace their questions with YAML placeholders.
