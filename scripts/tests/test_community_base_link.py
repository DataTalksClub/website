"""Helper checks for the community-base link tooling and source guard (D0.1a)."""
...
def commit_all(repo: Path, root: Path, message: str) -> None:
    ...
    subprocess.run(
        ["git", "-C", str(repo), "commit", "-q", "-m", message], check=True, env=env, capture_output=True
    )
...
class LinkRoundTripTests(SimpleTestCase):
    ...
    def test_link_appends_source_table_and_snapshots_originals(self):
        ...
        self.assertEqual((self.snapshot / "pyproject.toml.orig").read_bytes(), self.original_pyproject)
    ...
    def test_link_inserts_into_existing_sources_table(self):
        existing = (
            PYPROJECT + '\n[tool.uv.sources]\nsome-package = { path = "some_package", editable = true }\n'
        )
        ...
        self.assertIn("some-package", text)
...
class SourceGuardTests(SimpleTestCase):
    """Negative pin fixtures: everything but the tagged release fails closed."""
    ...
    def test_local_path_requirement_fails(self):
        write(
            self.repo / "pyproject.toml",
            PYPROJECT.replace(
                "community-base @ git+https://github.com/DataTalksClub/community-base@v0.3.0",
                "community-base @ file://../community-base",
            ),
        )
        self.assertEqual(self.check(), 1)

    def test_branch_ref_requirement_fails(self):
        write(
            self.repo / "pyproject.toml",
            PYPROJECT.replace(
                "@v0.3.0",
    ...
    def test_pyproject_lock_tag_mismatch_fails(self):
        write(self.repo / "uv.lock", LOCK.replace("rev=v0.3.0", "rev=v0.4.0"))
        self.assertEqual(self.check(), 1)
        self.assertIn("mismatch", self.detained_stderr)
