"""Pinned public route contracts for the editorial content sources.

Each row records one exact public path from a generated source site — the main site, docs, FAQ,
Podwiki — or from the adopted course-platform URLconfs, together with the repository, revision and
source path it came from. ``content.inventory`` filters these to base-path route contracts and the
content pipeline binds every ingested document and asset to one of them.

The pinned artifacts live in ``_docs/compatibility/`` and are the input, not a comparison target:
nothing here fetches, crawls or diffs the old site.
"""
...
class ReviewState(StrEnum):
    """Explicit review authority for a route-contract classification."""
...
def value_requires_redaction(key: str, value: str) -> bool:
    """Detect a credential-shaped or personal query/fragment value."""
...
def fragment_requires_redaction(value: str) -> bool:
    """Detect secrets in ordinary and query-shaped URL fragments."""
