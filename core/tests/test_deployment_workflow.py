...
class DeploymentWorkflowContractTests(SimpleTestCase):
    ...
    def test_runbook_documents_build_once_records_reuse_and_safe_auto_capture(self) -> None:
        ...
        self.assertIn("## Database URL rotation sync", runbook)
        self.assertIn("`RDS-EVENT-0016`", runbook)
        self.assertIn("`website-sandbox/database-url`", runbook)
        self.assertIn("sandbox/website-database-url-sync", runbook)
        self.assertIn("`website-sandbox-web-password-auth-failed`", runbook)
        self.assertIn("`website-sandbox-worker-password-auth-failed`", runbook)
