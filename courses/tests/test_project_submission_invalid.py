...
class ProjectSubmissionInvalidExistingViewTestCase(ProjectSubmissionViewTestBase):
    def test_existing_submission_post_does_not_fetch_link_status(self):
        self.prepare_project_with_learning_cap()
        self.invalid_link_existing_submission()
        ...
        self.assert_project_submission_matches(self.get_project_submission(), data)
...
class ProjectSubmissionFaqUrlViewTestCase(ProjectSubmissionViewTestBase):
    ...
    def test_project_submission_rejects_non_faq_contribution_url(self, mock_get, mock_head):
    ...
    @mock.patch("requests.head")
    @mock.patch("requests.get")
    def test_project_submission_accepts_faq_issue_url(self, mock_get, mock_head):
        self.mock_url_check_status(mock_get, mock_head, 200)
        self.enable_faq_contribution_field()
...
class ProjectSubmissionInvalidNewViewTestCase(ProjectSubmissionViewTestBase):
    def test_project_submission_post_safe_link_is_not_remote_status_checked(self):
