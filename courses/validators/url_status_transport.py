"""HTTP transport, redirect-rejection, SSRF guards, and FAQ URL cleaning.

This module carries the actual security-sensitive logic behind the field
URL validators: it makes (or bounds) synchronous HTTP requests, rejects
redirects to private/metadata destinations, and validates the shared FAQ
contribution URL shape.

It is deliberately kept out of ``courses.validators.custom_url_validators``,
the thin module Django migrations import by reference for the field-attached
validators (see that module's docstring). This module is loaded lazily, at
call time, from there -- never at migration-load time.
"""
...
def check_url_status(url, get_method=None, code=None, params=None):
    """The real implementation behind ``validate_url_200``.

    Kept here, away from the migration-serialized thin wrapper, so a
    ``migrate`` replay never imports ``requests`` or ``core.security``.
    """
